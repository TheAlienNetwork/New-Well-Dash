...
export class EmailService {
  ...
  generateGammaPlotImage(): string {
    try {
      ...
      // Get gamma data from the window object (this is a simplification)
      // In a real implementation, we would pass the gamma data as a parameter
      ...
      const depths = gammaData.map(d => d.depth);
      const values = gammaData.map(d => d.value);
      const minDepth = Math.min(...depths);
      const maxDepth = Math.max(...depths);
      const maxValue = Math.max(...values, 100); // At least 100 as max
      ...
      ctx.textAlign = 'center';
      ctx.fillText('Depth (ft)', canvas.width / 2, canvas.height - 15);
      
      ctx.save();
      ctx.translate(15, canvas.height / 2);
      ctx.rotate(-Math.PI / 2);
      ctx.fillText('Gamma API', 0, 0);
      ctx.restore();
      ...
      if (gammaData.length > 0) {
        // Draw line
        ctx.beginPath();
        gammaData.forEach((point, i) => {
          const x = padding.left + ((point.depth - minDepth) / (maxDepth - minDepth)) * chartWidth;
          const y = canvas.height - padding.bottom - (point.value / maxValue) * chartHeight;
        ...
        // Line style - neon green with glow
        ctx.strokeStyle = '#4ade80'; // Lime green
        ctx.lineWidth = 3;
        ctx.stroke();
        
        // Add slight gradient under the line
        const gradient = ctx.createLinearGradient(0, padding.top, 0, canvas.height - padding.bottom);
        gradient.addColorStop(0, 'rgba(74, 222, 128, 0.2)'); // Lime green with low opacity
    ...
    return `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; background-color: #0f172a; color: #e2e8f0; border-radius: 10px;">
        ...
        <div style="background-color: #1e293b; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #0e7490;">
          ...
          <div style="text-align: center; margin-top: 10px; background-color: #0f1629; padding: 15px; border-radius: 8px;">
            <img src="${gammaImageUrl}" alt="Gamma Plot" style="width: 100%; max-width: 600px; border-radius: 4px; border: 1px solid #0e7490;" />
          </div>
          <div style="margin-top: 10px; display: flex; justify-content: center;">
            <div style="width: 15px; height: 15px; background-color: #4ade80; border-radius: 50%; margin-right: 5px;"></div>
            <span style="color: #4ade80; font-family: monospace;">Gamma Points</span>
          </div>
        </div>
        ` : ''}
